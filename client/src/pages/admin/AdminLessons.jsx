...
const AdminLessons = () => {
  ...
  const handleDeleteLesson = async (lessonId) => {
    if (window.confirm('Bu dersi silmek istediğinize emin misiniz?')) {
      try {
        ...
        alert('Ders silinirken hata oluştu: ' + (error.data?.message || error.message));
      }
    }
  };
  ...
  const handleSubmitAdd = async (e) => {
    ...
    try {
      ...
      alert('Ders eklenirken hata oluştu: ' + (error.data?.message || error.message));
    }
  };
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      ...
      alert('Ders güncellenirken hata oluştu: ' + (error.data?.message || error.message));
    }
  };
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Ders ara (ad, kod, öğretmen)..."
                value={search}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          {lessonsLoading ? (
            ...
            <div className="text-center py-12">
              <p className="text-red-600">Hata: {lessonsError.data?.message || lessonsError.message}</p>
            </div>
          ) : (
            <>
                    ...
                    <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                      <div>
                        <p className="text-sm text-gray-700">
                          <span className="font-medium">{lessonsData.pagination.totalLessons}</span> dersten{' '}
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <div className="text-center mb-6">
                      ...
                      <p className="text-gray-500">Ders Kodu: {lessonDetail.lesson.code}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-gray-900">
                            %{Math.round((lessonDetail.lesson.enrolledStudentsCount / lessonDetail.lesson.maxCapacity) * 100)}
                          </p>
                        </div>
                      </div>
                    ...
                    {/* Kapasite Durumu */}
                    <div className="mt-6 border-t border-gray-200 pt-6">
                      <h5 className="text-lg font-semibold text-gray-900 mb-4">Kapasite Durumu</h5>
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600">Kayıtlı Öğrenci</span>
                          <span className="font-medium">{lessonDetail.lesson.enrolledStudentsCount}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600">Boş Yer</span>
                          <span className="font-medium">{lessonDetail.lesson.maxCapacity - lessonDetail.lesson.enrolledStudentsCount}</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-green-600 h-2 rounded-full transition-all duration-300"
                            style={{ 
                              width: `${(lessonDetail.lesson.enrolledStudentsCount / lessonDetail.lesson.maxCapacity) * 100}%` 
                            }}
                          ></div>
                        </div>
                        <div className="text-center">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                            getCapacityStatus(lessonDetail.lesson.enrolledStudentsCount, lessonDetail.lesson.maxCapacity).color === 'green' 
                              ? 'bg-green-100 text-green-800' 
                              : getCapacityStatus(lessonDetail.lesson.enrolledStudentsCount, lessonDetail.lesson.maxCapacity).color === 'yellow'
                              ? 'bg-yellow-100 text-yellow-800'
                              : getCapacityStatus(lessonDetail.lesson.enrolledStudentsCount, lessonDetail.lesson.maxCapacity).color === 'orange'
                              ? 'bg-orange-100 text-orange-800'
                              : 'bg-red-100 text-red-800'
                          }`}>
                            {getCapacityStatus(lessonDetail.lesson.enrolledStudentsCount, lessonDetail.lesson.maxCapacity).text}
                          </span>
                        </div>
                      </div>
                    </div>
